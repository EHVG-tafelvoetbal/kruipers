const data = {
    ...
    2026: {
        ...
        02: {
            ...
            Jelmer: 7, // 04-02 (& Jasper) door Sander+Lars; // 04-02 (& Stefan) door Jasper+Lars; // 05-02 (& Ibrahim) door Beau+Reno; // 25-02 (& Sander) door Lars+Jasper;  // 26-02 (& Mike) door Beau en Jelmer;  // 26-02 (& Stefan) door Beau en Mike;  // 26-02 (& Stefan) door Beau en Mike;  // 26-02 (& Marijn) door Beau en Mike;
            Stefan: 7, // 04-02 (& Jelmer) door Jasper+Lars; // 09-02 (& Jasper) door Ricardo en Lars; // 16-02 (& Marijn) door Beau en Jasper; // 18-02 (& Ibrahim) door Lars & Ricardo;  // 26-02 (& Jelmer) door Beau en Mike;  // 26-02 (& Jelmer) door Beau en Mike;
            Ibrahim: 2, // 05-02 (& Jelmer) door Beau+Reno; // 18-02 (& Stefan) door Lars & Ricardo;
            Rick: 1, // 13-02 (& Sander) door Lars+Ricardo;
            Sander: 2, // 13-02 (& Rick) door Lars+Ricardo; // 25-02 (& Jelmer) door Lars+Jasper;
        ...
        03: {
            Stefan: 12, // 01-03 (& Mike) door An+Jelmer; // 05-03 (& Mike) door Lars+Michel;  // 05-03 (& Jorik) door Lars+Jasper;  // 05-03 (& Jelmer) door Marijn & Beau; // 09-03 (& Jelmer) door Boyd & Lars; // 16-03 (& Jorik) door Beau & Michel; // 17-03 (& Marijn) door Beau & Ricardo;  // 19-03 (& Jelmer) door Marijn & Mike; // 24-03 (& Mike) door Beau & Ricardo; // 27-03 (& Jelmer) door Beau & Lars;// 31-03 (& Mike) door Beau+Boyd;// 31-03 (& Mike) door Beau+Marijn;
            Mike: 6,  // 01-03 (& Stefan) door An+Jelmer; // 05-03 (& Stefan) door Lars+Michel;  // 24-03 (& An) door Beau & Jorik; // 24-03 (& Stefan) door Beau & Ricardo;;// 31-03 (& Stefan) door Beau+Boyd;// 31-03 (& Stefan) door Beau+Marijn
            ...
            Sander: 1,  // 19-03 (& Jelmer) door Lars & Ricardo;
            An: 1, // 24-03 (& Mike) door Beau & Jorik;
        ...
        07: {
            Mike: 4, // 09-07 (& Stefan) door Beau+Marijn; // 13-07 (& Stefan) door Beau+Jorik; // 13-07 (& Stefan) door Marijn+Jorik; // 14-07 (& Stefan) door Marijn+Beau
            Stefan: 5, // 09-07 (& Mike) door Beau+Marijn; // 09-07 (& An) door Beau+Mike; // 13-07 (& Mike) door Beau+Jorik; // 13-07 (& Mike) door Marijn+Jorik; // 14-07 (& Mike) door Marijn+Beau
